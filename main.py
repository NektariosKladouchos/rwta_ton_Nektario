...
with tab_calc:
    ...
    with right:
        ...
        if not v_name or not v_job or not v_addr:
            ...
            error_to_show = "❌ ΜΗ ΣΥΜΒΑΤΟ ΣΥΣΤΗΜΑ VRV (Η ΕΠΙΛΟΓΗ 'ΑΛΛΗ' ΔΕΝ ΥΠΟΣΤΗΡΙΖΕΤΑΙ)"
        elif h_type == "VRV/VRF" and c_type == "VRV/VRF" and hb != cb:
            error_to_show = "❌ ΛΑΘΟΣ: ΠΡΕΠΕΙ ΝΑ ΕΧΕΤΕ ΤΗΝ ΙΔΙΑ ΜΑΡΚΑ VRV ΣΕ Θ/Ψ"
        ...
        if not error_to_show:
            ...
            if is_common:
                ...
                # Εμφάνιση Brand μόνο αν είναι VRV
                desc = f"{h_type} ({hb if 'VRV' in h_type else ''}) [Κοινό]"
                hvac_details.append({"n": desc.replace("()", ""), "q": h_qty, "p": hvac_cost})
            else:
                ...
                if h_qty > 0 and h_type != "Κανένα":
                    ...
                    desc = f"Θ: {h_type} ({hb if 'VRV' in h_type else ''})"
                    hvac_details.append({"n": desc.replace("()", ""), "q": h_qty, "p": p})
                ...
                if c_qty > 0 and c_type != "Κανένα":
                    ...
                    desc = f"Ψ: {c_type} ({cb if 'VRV' in c_type else ''})"
                    hvac_details.append({"n": desc.replace("()", ""), "q": c_qty, "p": p})
        ...
        with st.expander("🏆 10 ΛΟΓΟΙ ΕΠΙΛΟΓΗΣ"):
            st.write("1. Retrofit τεχνολογία | 2. Z-Wave Αξιοπιστία | 3. Εξοικονόμηση 30% | 4. App Έλεγχος | 5. Σενάρια | 6. Επεκτασιμότητα | 7. Συμβατότητα HVAC | 8. Ελληνική Υποστήριξη | 9. Design | 10. Αξία Ακινήτου")
