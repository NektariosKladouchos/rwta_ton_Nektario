...
# --- COMPACT & GRAPHICAL CSS ---
st.markdown("""
    <style>
    .stApp { background-color: #f8f9fa; }
    /* Σφίξιμο των κενών αριστερά */
    .stNumberInput, .stSelectbox, .stTextInput, .stRadio { margin-bottom: -18px !important; }
    .stMarkdown h3 { font-size: 16px !important; margin-bottom: -10px !important; color: #1E3A8A; }
    .stMarkdown p { font-size: 13px !important; margin-bottom: -5px !important; }
    
    /* Spacer για ευθυγράμμιση Display με 2α */
    .spacer { height: 260px; } 
...
with tab_calc:
    ...
    with left:
        ...
        led = st.number_input("LED Dim (ταινίες)", min_value=0)
        ...
        h_type = st.selectbox("Επιλογή", h_list, key="ht")
        ...
        h_brand = "Daikin"
        if h_type == "VRV/VRF": h_brand = st.selectbox("Brand (Θ)", BRANDS, key="hb")
        ...
        c_type = st.selectbox("Επιλογή ", c_list, key="ct")
        ...
        c_brand = "Daikin"
        if c_type == "VRV/VRF": c_brand = st.selectbox("Brand (Ψ)", BRANDS, key="cb")

        st.markdown("### 🪟 5. ΡΟΛΑ / ΤΕΝΤΕΣ")
        shutt = st.number_input("Σύνολο Ρολών", min_value=0)
    ...
    with right:
        ...
        elif h_type == "Ενδοδαπέδια" and c_type == "Ενδοδαπέδια Δροσισμός" and h_qty != c_qty:
        ...
        if error_msg:
            ...
            hubs_cost = 0; hub_rows = []; hub_q = 0
            if base_count <= 37:
                hubs_cost = PRICES["hub_small"]; hub_q = 1; hub_rows.append(f"{'Κεντρική μονάδα (40)':<35} | 1       | {hubs_cost:9.2f}€")
            elif base_count <= 97:
                hubs_cost = PRICES["hub_large"]; hub_q = 1; hub_rows.append(f"{'Κεντρική μονάδα (100)':<35} | 1       | {hubs_cost:9.2f}€")
            elif base_count <= 130:
                hubs_cost = PRICES["hub_large"] + PRICES["hub_small"]; hub_q = 2
                ...
                hubs_cost = PRICES["hub_large"] * 2; hub_q = 2
                hub_rows.append(f"{'Κεντρική μονάδα (100)':<35} | 2       | {hubs_cost:9.2f}€")
            
            total_dev = base_count + hub_q
            if total_dev > 230: st.error(f"❌ ΣΦΑΛΜΑ: {total_dev} ΣΥΣΚΕΥΕΣ (ΟΡΙΟ 230)")
            else:
                ...
                if on_off > 0: res += f"{'Γραμμές On/Off':<35} | {on_off:<7} | {on_off*63.92:9.2f}€\n"
                if double > 0: res += f"{'Διπλές (Κομιτατέρ)':<35} | {double:<7} | {double*63.92:9.2f}€\n"
                ...
                res += f"{'ΚΟΣΤΟΣ ΠΡΟΓΡΑΜΜΑΤΙΣΜΟΥ (20%):':<46} {mat_sum*0.20:11.2f}€\n"
                res += f"{'ΦΠΑ 24%:':<46} {(mat_sum*1.20)*0.24:11.2f}€\n"
                res += f"{'='*68}\n"
                res += f"{'ΓΕΝΙΚΟ ΣΥΝΟΛΟ:':<46} {(mat_sum*1.20)*1.24:11.2f}€\n"
                res += f"{'='*68}"
