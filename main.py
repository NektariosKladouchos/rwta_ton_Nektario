    ...
    with right:
        ...
        if error:
            ...
            if is_common:
                ...
                h_details.append({"n": f"{h_type} {hb if h_type=='VRV/VRF' else ''} (Κοινό)", "q": h_qty, "p": h_cost_hvac})
            else:
            ...
            total_sum_val = (max(0,on_off)*63.92) + (double*63.92) + (dim220*63.92) + (dim110*52.0) + (led*63.92) + (dali*160.0) + (shutt*63.92) + h_cost_hvac + h_total + e_cost + (95 if heater else 0)
