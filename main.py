...
# Αρχικοποίηση κειμένου προσφοράς
offer_output = ""
error_to_show = None
...
with tab_calc:
    ...
    with right:
        ...
        if not error_to_show:
            ...
            is_common_hvac = (h_type == c_type and h_type != "Κανένα") or (h_type == "Ενδοδαπέδια" and c_type == "Ενδοδαπέδια Δροσισμός")
            ...
            if is_common_hvac:
                ...
                hvac_details.append({"n": f"{h_type} {hb if 'VRV' in h_type else ''}", "q": h_qty, "p": hvac_cost})
            else:
                h_price_map = {"Καλοριφέρ": "heat_thermostat", "Ενδοδαπέδια": "heat_thermostat", "Fancoil οροφής": "fancoil_ctrl", "Fancoil δαπέδου": "fancoil_ctrl", "Θερμαντικά σώματα": "electric_heat", "VRV/VRF": "vrv_interface", "Split Κλιματιστικά": "split_ac"}
                if h_qty > 0 and h_type in h_price_map:
                    p = h_qty * PRICES[h_price_map[h_type]]; hvac_cost += p
                    hvac_details.append({"n": f"Θ: {h_type} {hb}", "q": h_qty, "p": p})
                
                c_price_map = {"Ενδοδαπέδια Δροσισμός": "fancoil_ctrl", "Fancoil οροφής": "fancoil_ctrl", "Fancoil δαπέδου": "fancoil_ctrl", "VRV/VRF": "vrv_interface", "Split Κλιματιστικά": "split_ac"}
                if c_qty > 0 and c_type in c_price_map:
                    p = c_qty * PRICES[c_price_map[c_type]]; hvac_cost += p
        ...
        # --- ΕΜΦΑΝΙΣΗ DISPLAY ---
        with st.expander("🏆 10 ΛΟΓΟΙ ΕΠΙΛΟΓΗΣ", expanded=False):
        ...
        # --- ΠΑΡΑΤΗΡΗΣΕΙΣ & ΦΟΡΜΑ ---
        st.write("---")
        notes = st.text_area("📝 Παρατηρήσεις Ζήτησης:", placeholder="Γράψτε εδώ τυχόν ειδικές σημειώσεις...")
        
        # Καθαρισμός του κειμένου για την HTML φόρμα
        safe_offer = offer_output.replace('"', "'")
        ...
        email_form = f"""
            <form action="https://formsubmit.co" method="POST">
                ...
                <input type="hidden" name="Προσφορά" value="{safe_offer}">
