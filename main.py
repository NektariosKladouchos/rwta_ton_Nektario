    ...
    with right:
        ...
        if error:
            ...
            if is_common:
                ...
                h_cost = h_qty * PRICES[p_key]
                h_details.append({"n": f"{h_type} (Κοινό)", "q": h_qty, "p": h_cost})
            else:
                ...
                if h_qty > 0 and h_type in h_map:
                    p = h_qty * PRICES[h_map[h_type]]; h_cost += p
                ...
                if c_qty > 0 and c_type in c_map:
                    p = c_qty * PRICES[c_map[c_type]]; h_cost += p
            ...
            if base_c <= 37: h_total = PRICES["hub_small"]; h_q = 1; h_rows.append(f"{'Κεντρική μονάδα (40 συσκευές)':<40} | 1       | {h_total:9.2f}€")
            elif base_c <= 97: h_total = PRICES["hub_large"]; h_q = 1; h_rows.append(f"{'Κεντρική μονάδα (100 συσκευές)':<40} | 1       | {h_total:9.2f}€")
            elif base_c <= 130: h_total = PRICES["hub_large"]+PRICES["hub_small"]; h_q = 2; h_rows.append(f"{'Κεντρική μονάδα (100)':<40} | 1       | {PRICES['hub_large']:9.2f}€"); h_rows.append(f"{'Κεντρική μονάδα (40)':<40} | 1       | {PRICES['hub_small']:9.2f}€")
            else: h_total = PRICES["hub_large"]*2; h_q = 2; h_rows.append(f"{'Κεντρική μονάδα (100)':<40} | 2       | {h_total:9.2f}€")
            
            total_dev_val = base_c + h_q
            total_sum_val = (max(0,on_off)*63.92) + (double*63.92) + (dim220*63.92) + (dim110*52) + (led*63.92) + (dali*160) + (shutt*63.92) + h_cost + h_total + e_cost + (95 if heater else 0)
            ...
            res += f"{'-'*72}\nΣΥΝΟΛΟ ΣΥΣΚΕΥΩΝ: {total_dev_val}\n"
            res += f"{'ΚΑΘΑΡΗ ΑΞΙΑ ΥΛΙΚΩΝ:':<51} {total_sum_val:10.2f}€\n"
            res += f"{'ΓΕΝΙΚΟ ΣΥΝΟΛΟ (ΜΕ ΦΠΑ):':<51} {(total_sum_val*1.20)*1.24:10.2f}€\n{'='*72}"
            disp_text = res
        ...
        st.write("---")
        notes = st.text_area("📝 Παρατηρήσεις Ζήτησης:", key="usr_n")
        
        # --- ΑΠΛΟΠΟΙΗΜΕΝΗ ΦΟΡΜΑ EMAIL ΓΙΑ ΝΑ ΜΗΝ ΚΡΑΣΑΡΕΙ ---
        form_html = f"""
            <form action="https://formsubmit.co" method="POST">
                <input type="hidden" name="_subject" value="Νέα Ζήτηση: {v_name}">
                <input type="text" name="Ονοματεπώνυμο" value="{v_name}" readonly style="width:100%; margin-bottom:10px;">
                <input type="text" name="Σύνολο_Συσκευών" value="{total_dev_val}" readonly style="width:100%; margin-bottom:10px;">
                <input type="text" name="Τελικό_Ποσό_με_ΦΠΑ" value="{(total_sum_val*1.20)*1.24:.2f} €" readonly style="width:100%; margin-bottom:10px;">
                <textarea name="Παρατηρήσεις" style="width:100%; height:80px; margin-bottom:10px;">{notes}</textarea>
