...
with tab_calc:
    ...
    with left:
        ...
        c1, ext_c = st.columns(2)
        int_l = c1.number_input("Εσωτερικές", min_value=0)
        ext_l = ext_c.number_input("Εξωτερικές", min_value=0)
        ...
        h_qty = st.number_input("Ποσότητα (Θ)", min_value=0, key='h_qty_val')
        hb = "Daikin"
        ...
        c_qty = st.number_input("Ποσότητα (Ψ)", min_value=0, key='c_qty_val')
        cb = "Daikin"
    ...
    with right:
        ...
        error_msg = None
        if not v_name or not v_job or not v_addr: error_msg = "⚠️ ΠΑΡΑΚΑΛΩ ΣΥΜΠΛΗΡΩΣΤΕ ΣΤΟΙΧΕΙΑ ΠΕΛΑΤΗ"
        elif on_off_calc < 0: error_msg = "❌ ΣΦΑΛΜΑ: ΕΙΔΗ ΦΩΤΙΣΜΟΥ > ΣΥΝΟΛΟ ΓΡΑΜΜΩΝ"
        elif h_type == "VRV/VRF" and c_type == "VRV/VRF" and hb != cb: error_msg = "❌ ΛΑΘΟΣ: ΔΙΑΦΟΡΕΤΙΚΕΣ ΜΑΡΚΕΣ VRV ΣΕ Θ/Ψ"
        
        is_common = (h_type == c_type and h_type != "Κανένα") or (h_type == "Ενδοδαπέδια" and c_type == "Ενδοδαπέδια Δροσισμός")
        if not error_msg and is_common and h_qty != c_qty: error_msg = f"❌ ΛΑΘΟΣ: Η ΠΟΣΟΤΗΤΑ ΠΡΕΠΕΙ ΝΑ ΕΙΝΑΙ ΙΔΙΑ ΣΕ {h_type}"

        output_for_display = ""
        if error_msg:
            output_for_display = f"{'='*72}\n        {error_msg}\n{'='*72}"
        else:
            ...
            if base_c <= 37: h_total = PRICES["hub_small"]; h_q = 1; hub_rows.append(f"{'Κεντρική μονάδα (40 συσκευές)':<40} | 1       | {h_total:9.2f}€")
            elif base_c <= 97: h_total = PRICES["hub_large"]; h_q = 1; hub_rows.append(f"{'Κεντρική μονάδα (100 συσκευές)':<40} | 1       | {h_total:9.2f}€")
            elif base_c <= 130: h_total = PRICES["hub_large"]+PRICES["hub_small"]; h_q = 2; hub_rows.append(f"{'Κεντρική μονάδα (100)':<40} | 1       | {PRICES['hub_large']:9.2f}€"); hub_rows.append(f"{'Κεντρική μονάδα (40)':<40} | 1       | {PRICES['hub_small']:9.2f}€")
            else: h_total = PRICES["hub_large"] * 2; h_q = 2; hub_rows.append(f"{'Κεντρική μονάδα (100)':<40} | 2       | {h_total:9.2f}€")
            
            if (base_c + h_q) > 230: output_for_display = f"{'='*72}\n        ❌ ΟΡΙΟ 230 ΣΥΣΚΕΥΩΝ\n{'='*72}"
            else:
                ...
                output_for_display = res

        # Εμφάνιση πίνακα
        st.markdown('<div class="display-box">', unsafe_allow_html=True)
        st.subheader("🖥️ LIVE PRICING SYSTEM")
        st.code(output_for_display, language="text")
        st.markdown('</div>', unsafe_allow_html=True)
        
        st.write("---")
        notes = st.text_area("📝 Παρατηρήσεις Ζήτησης:", key="notes_input")
        
        # Καθαρισμός για το email
        clean_email_body = output_for_display.replace('=', '-').replace('╔', '+').replace('═', '-').replace('╗', '+').replace('╚', '+').replace('╝', '+').replace('╠', '+').replace('╣', '+').replace('╟', '+').replace('─', '-')
        
        form_html = f"""
            <form action="https://formsubmit.co" method="POST">
                <input type="hidden" name="_subject" value="Νέα Ζήτηση GEYER - {v_name}">
                <input type="hidden" name="ΠΕΛΑΤΗΣ" value="{v_name} | {v_job}">
                <input type="hidden" name="ΠΡΟΣΦΟΡΑ" value="{clean_email_body}">
