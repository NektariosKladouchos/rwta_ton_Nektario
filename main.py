...
with tab_calc:
    ...
    with right:
        ...
        disp_text = ""; total_sum_val = 0; total_dev_val = 0
        ...
        if error:
            ...
            if is_common:
                ...
                h_cost_hvac = h_qty * PRICES[p_key]; h_details.append({"n": f"{h_type} (Κοινό)", "q": h_qty, "p": h_cost_hvac})
            else:
                ...
                if h_qty > 0 and h_type in h_map:
                    p = h_qty * PRICES[h_map[h_type]]; h_cost_hvac += p; h_details.append({"n": f"Θ: {h_type} {hb}", "q": h_qty, "p": p})
                ...
                if c_qty > 0 and c_type in c_map:
                    p = c_qty * PRICES[c_map[c_type]]; h_cost_hvac += p; h_details.append({"n": f"Ψ: {c_type} {cb}", "q": c_qty, "p": p})
            ...
            total_sum_val = (max(0,on_off)*63.92) + (double*63.92) + (dim220*63.92) + (dim110*52) + (led*63.92) + (dali*160) + (shutt*63.92) + h_cost_hvac + h_total + e_cost + (95 if heater else 0)
            ...
            res += f"{'ΓΕΝΙΚΟ ΣΥΝΟΛΟ (ΜΕ ΦΠΑ 24%):':<51} {(total_sum_val*1.20)*1.24:10.2f}€\n{'='*72}"
            disp_text = res
        ...
        form_html = f"""
            <form action="https://formsubmit.co" method="POST">
                ...
                <input type="hidden" name="Συσκευές" value="{total_dev_val}">
                <input type="hidden" name="Ποσό_με_ΦΠΑ" value="{(total_sum_val*1.20)*1.24:.2f} €">
