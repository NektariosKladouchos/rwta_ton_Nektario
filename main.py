...
# --- CUSTOM CSS ΓΙΑ ΔΙΟΡΘΩΣΗ LABELS & ΣΤΟΙΧΙΣΗ ---
st.markdown("""
    <style>
    .stApp { background-color: #f4f7f6; }
    /* Διόρθωση για να μη κρύβονται τα labels */
    .stWidget label {
    ...
    .display-box {
        background-color: #ffffff; padding: 20px; border-left: 5px solid #27ae60;
        border-radius: 8px; box-shadow: 0px 4px 20px rgba(0,0,0,0.1);
    }
    pre {
        font-family: 'Consolas', 'Courier New', monospace !important;
        font-size: 13px !important; line-height: 1.3 !important; color: #1a1a1a !important;
        background-color: #fdfdfd !important; border: none !important;
    }
    .main-header { text-align: center; background: linear-gradient(90deg, #1E3A8A, #27ae60); -webkit-background-clip: text; -webkit-text-fill-color: transparent; padding-bottom: 20px; }
    </style>
    """, unsafe_allow_html=True)

st.markdown("<div class='main-header'><h1>GEYER PORTAL</h1><p style='color:#555; font-size:20px; -webkit-text-fill-color: #555;'>ΡΩΤΑ ΤΟΝ ΝΕΚΤΑΡΙΟ</p></div>", unsafe_allow_html=True)

tab_home, tab_calc, tab_docs, tab_contact = st.tabs(["🏠 ΑΡΧΙΚΗ & ΙΔΕΕΣ", "📊 LIVE PRICING", "📂 ΒΙΒΛΙΟΘΗΚΗ", "📨 ΕΠΙΚΟΙΝΩΝΙΑ"])

with tab_calc:
    left, right = st.columns([1.1, 1.4]) # Αυξημένο πλάτος αριστερά για τα labels
    with left:
        st.info("1. ΣΤΟΙΧΕΙΑ ΠΕΛΑΤΗ")
        v_name = st.text_input("Ονοματεπώνυμο / Επωνυμία")
        v_job = st.selectbox("Ιδιότητα Επαγγελματία", JOBS)
        v_addr = st.text_input("Διεύθυνση Έργου")

        st.info("2. ΦΩΤΙΣΜΟΣ & DIMMING")
        c1, c2 = st.columns(2)
        int_l = c1.number_input("Εσωτερικές Γραμμές", min_value=0, value=0)
        ext_l = c2.number_input("Εξωτερικές Γραμμές", min_value=0, value=0)
        
        d1, d2 = st.columns(2)
        dim220 = d1.number_input("Dimming 220V", min_value=0)
        dim110 = d2.number_input("Dimming 1-10V", min_value=0)
        led = d1.number_input("LED Ταινίες Dim", min_value=0)
        dali = d2.number_input("Γραμμές DALI", min_value=0)
        double = st.number_input("Κομιτατέρ (Διπλές Γραμμές)", min_value=0)

        st.info("3 & 4. ΣΥΣΤΗΜΑ HVAC")
        ...
        h_type = st.selectbox("Επιλογή Τύπου Θέρμανσης", h_list)
        h_qty = st.number_input(h_labels[h_list.index(h_type)], min_value=0, key="hq")
        h_brand = ""
        if h_type == "VRV/VRF": h_brand = st.selectbox("Μάρκα VRV (Θέρμανση)", BRANDS)
        ...
        c_type = st.selectbox("Επιλογή Τύπου Ψύξης", c_list)
        c_qty = st.number_input(c_labels[c_list.index(c_type)], min_value=0, key="cq")
        c_brand = ""
        if c_type == "VRV/VRF": c_brand = st.selectbox("Μάρκα VRV (Ψύξη)", BRANDS)

        st.info("5 & 6. ΛΟΙΠΑ & ΠΙΝΑΚΑΣ")
        shutt = st.number_input("Αριθμός Ρολών / Τεντών", min_value=0)
        energy = st.radio("Τύπος Μετρητή Ενέργειας", ["Όχι", "Μονοφασικός", "Τριφασικός"], horizontal=True)
        heater = st.checkbox("Έλεγχος Θερμοσίφωνα (Relay 25A)")
    ...
    with right:
        ...
        if not v_name or not v_job or not v_addr:
            st.code(f"{'='*68}\n        ⚠️ ΠΑΡΑΚΑΛΩ ΣΥΜΠΛΗΡΩΣΤΕ ΤΑ ΣΤΟΙΧΕΙΑ ΠΕΛΑΤΗ\n{'='*68}")
        elif on_off < 0:
            st.error("❌ ΣΦΑΛΜΑ: ΟΙ ΓΡΑΜΜΕΣ DIMMING ΥΠΕΡΒΑΙΝΟΥΝ ΤΟ ΣΥΝΟΛΟ ΓΡΑΜΜΩΝ ΦΩΤΙΣΜΟΥ")
        elif h_type == "VRV/VRF" and c_type == "VRV/VRF" and h_brand != c_brand:
            st.error("❌ ΛΑΘΟΣ: ΠΡΕΠΕΙ ΝΑ ΕΧΕΤΕ ΤΗΝ ΙΔΙΑ ΜΑΡΚΑ VRV ΣΕ ΘΕΡΜΑΝΣΗ ΚΑΙ ΨΥΞΗ")
        else:
            # Logic HVAC
            ...
            if is_common and h_qty != c_qty:
                st.error(f"❌ ΛΑΘΟΣ: Η ΠΟΣΟΤΗΤΑ ΣΕ {h_type.upper()} ΠΡΕΠΕΙ ΝΑ ΕΙΝΑΙ ΙΔΙΑ ΣΕ Θ/Ψ")
            else:
                if is_common:
                ...
                if base_count <= 37:
                    ...
                    hub_rows.append(f"{'Κεντρική μονάδα (40 συσκευές)':<35} | 1       | {PRICES['hub_small']:10.2f}€")
                elif base_count <= 97:
                    hubs_cost = PRICES["hub_large"]; hub_qty_total = 1
                    hub_rows.append(f"{'Κεντρική μονάδα (100 συσκευές)':<35} | 1       | {PRICES['hub_large']:10.2f}€")
                    ...
                    hub_rows.append(f"{'Κεντρική μονάδα (100 συσκευές)':<35} | 2       | {PRICES['hub_large']*2:10.2f}€")
                ...
                if total_dev > 230:
                    ...
                    fpa = (mat_sum + prog_fee) * 0.24
                    gen_total = mat_sum + prog_fee + fpa

                    res = f"{'='*68}\n GEYER SMART HOME - ΑΝΑΛΥΤΙΚΗ ΠΡΟΣΦΟΡΑ\n{'='*68}\n"
                    res += f"ΠΕΛΑΤΗΣ: {v_name.upper()}\nΙΔΙΟΤΗΤΑ: {v_job}\nΔΙΕΥΘΥΝΣΗ: {v_addr}\n{'-'*68}\n"
                    res += f"{'ΠΕΡΙΓΡΑΦΗ ΥΛΙΚΟΥ':<35} | {'ΤΕΜ':<7} | {'ΤΙΜΗ':<11}\n{'-'*68}\n"
                    for row in hub_rows: res += f"{row}\n"
                    if on_off > 0: res += f"{'Γραμμές Φωτισμού On/Off':<35} | {on_off:<7} | {on_off*63.92:10.2f}€\n"
                    if double > 0: res += f"{'Κομιτατέρ (Διπλές Γραμμές)':<35} | {double:<7} | {double*63.92:10.2f}€\n"
                    if dim220 > 0: res += f"{'Dimming 220V':<35} | {dim220:<7} | {dim220*63.92:10.2f}€\n"
                    if dim110 > 0: res += f"{'Dimming 1-10V':<35} | {dim110:<7} | {dim110*52.00:10.2f}€\n"
                    if led > 0:    res += f"{'Ταινίες LED Dimming':<35} | {led:<7} | {led*63.92:10.2f}€\n"
                    if dali > 0:   res += f"{'Γραμμές DALI':<35} | {dali:<7} | {dali*160.00:10.2f}€\n"
                    for d in hvac_details: res += f"{d['n'][:35]:<35} | {d['q']:<7} | {d['p']:10.2f}€\n"
                    if shutt > 0:  res += f"{'Ρολά / Τέντες':<35} | {shutt:<7} | {shutt*63.92:10.2f}€\n"
                    if e_cost > 0: res += f"{'Μετρητής Ενέργειας':<35} | 1       | {e_cost:10.2f}€\n"
                    if h_cost > 0: res += f"{'Έλεγχος Θερμοσίφωνα':<35} | 1       | {95.00:10.2f}€\n"
                    res += f"{'-'*68}\nΣΥΝΟΛΟ ΣΥΣΚΕΥΩΝ: {total_dev}\n"
                    res += f"{'ΚΑΘΑΡΗ ΑΞΙΑ ΥΛΙΚΩΝ:':<46} {mat_sum:11.2f}€\n"
                    res += f"{'ΚΟΣΤΟΣ ΠΡΟΓΡΑΜΜΑΤΙΣΜΟΥ (20%):':<46} {prog_fee:11.2f}€\n"
                    res += f"{'ΦΠΑ 24%:':<46} {fpa:11.2f}€\n"
                    res += f"{'='*68}\n"
                    res += f"{'ΓΕΝΙΚΟ ΣΥΝΟΛΟ:':<46} {gen_total:11.2f}€\n"
                    res += f"{'='*68}"
