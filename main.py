...
with tab_calc:
    ...
    with right:
        ...
        # --- ΥΠΟΛΟΓΙΣΜΟΙ ---
        final_disp = ""; h_details = []; t_dev = 0; t_sum = 0
        ...
        if not error:
            ...
            if is_exact:
                ...
                h_c = h_qty * PRICES[pk]
                h_details.append({"n": f"{h_type} [Κοινό]", "q": h_qty, "p": h_c})
            else:
                ...
                if h_qty > 0: p = h_qty * PRICES[h_m[h_type]]; h_c += p; h_details.append({"n": f"Θ:{h_type} {hb}", "q": h_qty, "p": p})
                if c_qty > 0: p = c_qty * PRICES[h_m.get(c_type, "fancoil_ctrl")]; h_c += p; h_details.append({"n": f"Ψ:{c_type} {cb}", "q": c_qty, "p": p})
            ...
            for d in h_details: res += f"{d['n'][:30]:<30} | {d['q']:<5} | {d['p']:8.2f}€\n"
            res += f"{'-'*55}\nΣΥΝΟΛΟ ΣΥΣΚΕΥΩΝ: {t_dev}\n"
            res += f"ΚΑΘΑΡΗ ΑΞΙΑ ΥΛΙΚΩΝ: {t_sum:10.2f}€\n"
            res += f"ΓΕΝΙΚΟ ΣΥΝΟΛΟ (ΜΕ ΦΠΑ & ΠΡΟΓΡ): {(t_sum*1.20)*1.24:10.2f}€"
            final_disp = res

        st.markdown('<div class="spacer"></div>', unsafe_allow_html=True)
        st.markdown('<div class="display-box">', unsafe_allow_html=True)
        st.subheader("🖥️ LIVE PRICING SYSTEM")
        st.code(final_disp if not error else error, language="text")
        ...
        if not error:
            ...
            # --- WORD GENERATION ---
            doc = Document()
            doc.add_heading(f'Προσφορά GEYER Smart Home - {v_name.upper()}', 0)
            doc.add_paragraph(f"Διεύθυνση Έργου: {v_addr}")
            doc.add_paragraph(f"Ιδιότητα: {v_job}")
            doc.add_paragraph("-" * 30)
            doc.add_paragraph(final_disp)
            ...
            st.download_button(
                label="📥 ΚΑΤΕΒΑΣΜΑ ΑΝΑΛΥΤΙΚΗΣ ΠΡΟΣΦΟΡΑΣ (WORD)",
                data=buffer,
                file_name=f"Offer_{v_name}.docx",
            ...
            email_summary = f"Client: {v_name} | Total: {(t_sum*1.20)*1.24:.2f} EUR | Devices: {t_dev}"
            
            form_html = f"""
                <form action="https://formsubmit.co" method="POST">
                    <input type="hidden" name="_subject" value="Νέα Ζήτηση GEYER - {v_name}">
                    <input type="hidden" name="Περίληψη" value="{email_summary}">
                    <input type="hidden" name="Αναλυτικά" value="{final_disp.replace('=', '-')}">
