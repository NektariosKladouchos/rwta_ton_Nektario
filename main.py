...
with tab_calc:
    ...
    with right:
        with st.expander("🏆 10 ΛΟΓΟΙ ΓΙΑ ΝΑ ΕΠΙΛΕΞΕΤΕ ΤΟ ΣΥΣΤΗΜΑ ΜΑΣ"):
            st.write("1. Retrofit | 2. Z-Wave Αξιοπιστία | 3. Εξοικονόμηση 30% | 4. App | 5. Σενάρια | 6. Επεκτασιμότητα | 7. Συμβατότητα | 8. Ελληνική Υποστήριξη | 9. Design | 10. Αξία Ακινήτου")
        ...
        # Αρχικοποίηση κενής προσφοράς
        final_offer_text = "⚠️ ΠΑΡΑΚΑΛΩ ΣΥΜΠΛΗΡΩΣΤΕ ΣΤΟΙΧΕΙΑ ΠΕΛΑΤΗ"
        error_found = False

        if not v_name or not v_job or not v_addr:
            error_found = True
        elif on_off_calc < 0:
            final_offer_text = "❌ ΣΦΑΛΜΑ: ΕΙΔΗ ΦΩΤΙΣΜΟΥ > ΣΥΝΟΛΟ ΓΡΑΜΜΩΝ"
            error_found = True
        elif h_type == "VRV/VRF" and c_type == "VRV/VRF" and hb != cb:
            final_offer_text = "❌ ΛΑΘΟΣ: ΔΙΑΦΟΡΕΤΙΚΕΣ ΜΑΡΚΕΣ VRV ΣΕ Θ/Ψ"
            error_found = True
        elif (h_type == "VRV/VRF" and hb == "Άλλη") or (c_type == "VRV/VRF" and cb == "Άλλη"):
            final_offer_text = "❌ ΜΗ ΣΥΜΒΑΤΟ ΣΥΣΤΗΜΑ VRV"
            error_found = True
        ...
        if not error_found:
            ...
            if is_common:
                ...
                if h_qty > 0 and h_type in h_map:
                    p = h_qty * PRICES[h_map[h_type]]; hvac_cost += p
                ...
                if c_qty > 0 and c_type in c_map:
                    p = c_qty * PRICES[c_map[c_type]]; hvac_cost += p
            ...
            if (base_count + h_q) > 230:
                final_offer_text = "❌ ΟΡΙΟ 230 ΣΥΣΚΕΥΩΝ ΥΠΕΡΒΛΗΘΗΚΕ"
            else:
                ...
                final_offer_text = res

        # --- ΕΜΦΑΝΙΣΗ DISPLAY ---
        st.markdown('<div class="display-box">', unsafe_allow_html=True)
        st.subheader("🖥️ LIVE PRICING SYSTEM")
        if error_found and "⚠️" in final_offer_text:
            st.code(f"{'='*72}\n        {final_offer_text}\n{'='*72}")
        else:
            st.code(final_offer_text, language="text")
