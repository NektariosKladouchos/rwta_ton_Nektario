...
st.markdown("""
    ...
    .send-btn {
        display: inline-block; padding: 15px 25px; background-color: #27ae60; color: white;
        text-decoration: none; border-radius: 5px; font-weight: bold; width: 100%; text-align: center;
    }
    </style>
...
with tab_calc:
    ...
    with right:
        with st.expander("🏆 10 ΛΟΓΟΙ ΕΠΙΛΟΓΗΣ"):
            st.write("1. Retrofit | 2. Z-Wave Αξιοπιστία | 3. Εξοικονόμηση 30% | 4. App | 5. Σενάρια | 6. Επεκτασιμότητα | 7. Συμβατότητα | 8. Υποστήριξη | 9. Design | 10. Αξία Ακινήτου")
        ...
        # --- ΠΑΡΑΓΩΓΗ MAILTO LINK ---
        subject = f"Ζήτηση GEYER SMART HOME - {v_name}"
        body = f"Στοιχεία Ζήτησης:\n{disp_text}\n\nΠΑΡΑΤΗΡΗΣΕΙΣ:\n{user_notes}"
        mailto_link = f"mailto:kladouxos@geyer.gr?subject={urllib.parse.quote(subject)}&body={urllib.parse.quote(body)}"
        
        st.markdown(f'<a href="{mailto_link}" class="send-btn">📩 Αποστολή Ζήτησης στον Νεκτάριο</a>', unsafe_allow_html=True)
