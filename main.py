...
with tab_calc:
    ...
    with right:
        with st.expander("🏆 10 ΛΟΓΟΙ ΕΠΙΛΟΓΗΣ"):
            st.write("1. Retrofit | 2. Z-Wave | 3. Εξοικονόμηση | 4. App | 5. Σενάρια | 6. Επεκτασιμότητα | 7. Συμβατότητα | 8. Υποστήριξη | 9. Design | 10. Αξία")
        ...
        elif (h_type == "VRV/VRF" and hb == "Άλλη") or (c_type == "VRV/VRF" and cb == "Άλλη"): error = "❌ ΜΗ ΣΥΜΒΑΤΟ VRV ('Άλλη')"
        ...
        if error:
            ...
            if (base_c + h_q) > 230: display_text = f"{'='*72}\n        ❌ ΟΡΙΟ 230 ΣΥΣΚΕΥΩΝ\n{'='*72}"
            else:
                m_sum = (max(0,on_off)*63.92) + (double*63.92) + (dim220*63.92) + (dim110*52) + (led*63.92) + (dali*160) + (shutt*63.92) + hvac_cost + h_tot + e_cost + (95 if heater else 0)
                ...
                res += f"{'ΦΠΑ 24%:':<51} {m_sum*0.24:10.2f}€\n"
                res += f"{'ΓΕΝΙΚΟ ΣΥΝΟΛΟ:':<51} {m_sum*1.24:10.2f}€\n"
                res += f"{'='*72}"
                display_text = res
        ...
        st.write("---")
        user_notes = st.text_area("📝 Παρατηρήσεις Ζήτησης:", key="usr_notes")
        ...
        form_html = f"""
            <form action="https://formsubmit.co" method="POST">
                ...
                <textarea name="Προσφορά_Λεπτομέρειες" style="display:none;">{display_text}</textarea>
                <textarea name="Παρατηρήσεις" style="display:none;">{user_notes}</textarea>
