...
with tab_calc:
    ...
    with right:
        ...
        # Container για το display για να μην βγαίνει ποτέ διπλό
        ...
        if not error_msg and is_common_hvac and h_qty != c_qty:
            error_msg = f"❌ ΛΑΘΟΣ: Η ΠΟΣΟΤΗΤΑ ΠΡΕΠΕΙ ΝΑ ΕΙΝΑΙ ΙΔΙΑ ΣΕ Θ/Ψ"

        if error_msg:
            display_container.code(f"{'='*72}\n        {error_msg}\n{'='*72}")
            final_email_content = error_msg
        else:
            # Υπολογισμοί
            ...
            if (base_count + h_q) > 230: display_container.error(f"❌ ΟΡΙΟ 230 ΣΥΣΚΕΥΩΝ")
            else:
                ...
                display_container.code(res, language="text")
                final_email_content = res
