...
with tab_calc:
    ...
    with right:
        ...
        with st.expander("🏆 10 ΛΟΓΟΙ ΓΙΑ ΝΑ ΕΠΙΛΕΞΕΤΕ ΤΟ ΣΥΣΤΗΜΑ ΜΑΣ"):
            st.markdown("1. Retrofit | 2. Z-Wave | 3. Εξοικονόμηση 30% | 4. App | 5. Σενάρια | 6. Επεκτασιμότητα | 7. Συμβατότητα | 8. Ελληνική Υποστήριξη | 9. Design | 10. Αξία Ακινήτου")
        ...
        if error_msg:
            ...
            if total_dev > 230: st.error(f"❌ ΟΡΙΟ 230 ΣΥΣΚΕΥΩΝ")
                ...
                final_res_display = f"{'='*72}\n GEYER SMART HOME - ΑΝΑΛΥΤΙΚΗ ΠΡΟΣΦΟΡΑ\n{'='*72}\n"
                final_res_display += f"ΠΕΛΑΤΗΣ: {v_name.upper()} | {v_job}\nΔΙΕΥΘΥΝΣΗ: {v_addr}\n{'-'*72}\n"
                final_res_display += f"{'ΠΕΡΙΓΡΑΦΗ ΥΛΙΚΟΥ':<40} | {'TEM':<7} | {'ΤΙΜΗ':<10}\n{'-'*72}\n"
                for row in hub_rows: final_res_display += row + "\n"
                if on_off_calc > 0: final_res_display += f"{'Γραμμές Φωτισμού On/Off':<40} | {on_off_calc:<7} | {on_off_calc*63.92:9.2f}€\n"
                if double > 0: final_res_display += f"{'Διπλές Γραμμές (Κομιτατέρ)':<40} | {double:<7} | {double*63.92:9.2f}€\n"
                if dim220 > 0: final_res_display += f"{'Dimming 220V':<40} | {dim220:<7} | {dim220*63.92:9.2f}€\n"
                if dim110 > 0: final_res_display += f"{'Dimming 1-10V':<40} | {dim110:<7} | {dim110*52.00:9.2f}€\n"
                if led > 0:    final_res_display += f"{'Ταινίες LED Dimming':<40} | {led:<7} | {led*63.92:9.2f}€\n"
                if dali > 0:   final_res_display += f"{'Γραμμές DALI':<40} | {dali:<7} | {dali*160.00:9.2f}€\n"
                for d in hvac_details: final_res_display += f"{d['n'][:40]:<40} | {d['q']:<7} | {d['p']:9.2f}€\n"
                if shutt > 0:  final_res_display += f"{'Ρολά / Τέντες':<40} | {shutt:<7} | {shutt*63.92:9.2f}€\n"
                if e_cost > 0: final_res_display += f"{'Μετρητής Ενέργειας':<40} | 1       | {e_cost:9.2f}€\n"
                if h_cost > 0: final_res_display += f"{'Έλεγχος Θερμοσίφωνα':<40} | 1       | {h_cost:9.2f}€\n"
                final_res_display += f"{'-'*72}\nΣΥΝΟΛΟ ΣΥΣΚΕΥΩΝ: {total_dev}\n"
                final_res_display += f"{'ΚΑΘΑΡΗ ΑΞΙΑ ΥΛΙΚΩΝ:':<51} {mat_sum:10.2f}€\n"
                final_res_display += f"{'ΚΟΣΤΟΣ ΠΡΟΓΡΑΜΜΑΤΙΣΜΟΥ (20%):':<51} {mat_sum*0.20:10.2f}€\n"
                final_res_display += f"{'ΦΠΑ 24%:':<51} {(mat_sum*1.20)*0.24:10.2f}€\n"
                final_res_display += f"{'='*72}\n{'ΓΕΝΙΚΟ ΣΥΝΟΛΟ:':<51} {(mat_sum*1.20)*1.24:10.2f}€\n{'='*72}"
                st.code(final_res_display, language="text")
