...
# --- CUSTOM CSS ΓΙΑ STICKY DISPLAY & ΣΤΟΙΧΙΣΗ ---
st.markdown("""
    <style>
...
with tab_calc:
    ...
    with right:
        ...
        if not v_name or not v_job or not v_addr:
            ...
            dev_count = max(0, on_off) + double + dim220 + dim110 + led + dali + shutt + h_qty + (1 if e_cost > 0 else 0) + (1 if heater else 0)
            
            if dev_count > 230:
                st.error(f"❌ ΣΦΑΛΜΑ: {dev_count} ΣΥΣΚΕΥΕΣ. ΤΟ ΟΡΙΟ ΕΙΝΑΙ 230.")
            else:
                ...
                res += f"{'ΠΕΡΙΓΡΑΦΗ ΥΛΙΚΟΥ':<32} | {'TEM':<4} | {'TIMH':<10}\n{'-'*62}\n"
                res += f"{'Κεντρική Μονάδα Hub':<32} | 1    | {hubs_cost:9.2f}€\n"
                if on_off > 0: res += f"{'Φωτισμός On/Off':<32} | {on_off:<4} | {on_off*63.92:9.2f}€\n"
                if double > 0: res += f"{'Κομιτατέρ (Διπλές)':<32} | {double:<4} | {double*63.92:9.2f}€\n"
                if dim220 > 0: res += f"{'Dimming 220V':<32} | {dim220:<4} | {dim220*63.92:9.2f}€\n"
                if dim110 > 0: res += f"{'Dimming 1-10V':<32} | {dim110:<4} | {dim110*52.00:9.2f}€\n"
                if led > 0:    res += f"{'Ταινίες LED Dimming':<32} | {led:<4} | {led*63.92:9.2f}€\n"
                if dali > 0:   res += f"{'Γραμμές DALI':<32} | {dali:<4} | {dali*160.00:9.2f}€\n"
                for d in hvac_details: res += f"{d['n'][:32]:<32} | {d['q']:<4} | {d['p']:9.2f}€\n"
                if shutt > 0:  res += f"{'Ρολά / Τέντες':<32} | {shutt:<4} | {shutt*63.92:9.2f}€\n"
                if e_cost > 0: res += f"{'Μετρητής Ενέργειας':<32} | 1    | {e_cost:9.2f}€\n"
                if heater:     res += f"{'Έλεγχος Θερμοσίφωνα':<32} | 1    | {95.00:9.2f}€\n"
                res += f"{'-'*62}\nΣΥΝΟΛΟ ΣΥΣΚΕΥΩΝ: {dev_count}\n"
                res += f"ΚΑΘΑΡΗ ΑΞΙΑ ΥΛΙΚΩΝ: {mat_sum:27.2f}€\n"
                res += f"ΑΞΙΑ ΜΕ ΦΠΑ 24%: {mat_sum*1.24:30.2f}€\n"
                res += f"ΚΟΣΤΟΣ ΠΡΟΓΡΑΜΜΑΤΙΣΜΟΥ (20%): {mat_sum*0.20:21.2f}€\n{'='*62}"
                st.text(res)
