...
st.markdown("""
    ...
    /* Spacer για ευθυγράμμιση με το 2α */
    .spacer { height: 280px; } 
...
with tab_calc:
    ...
    with right:
        ...
        # Χρήση st.empty για να μην εμφανίζεται ποτέ διπλό
        ...
        if error_msg:
            display_area.code(f"{'='*72}\n        {error_msg}\n{'='*72}")
            final_res_text = error_msg
        else:
            # Safe HVAC Pricing Logic
            ...
            if is_common_hvac:
                ...
                if h_qty > 0 and h_type != "Κανένα":
                    p_key = h_price_keys[h_list.index(h_type)]
                    if p_key:
                        p = h_qty * PRICES[p_key]; hvac_cost += p
                ...
                if c_qty > 0 and c_type != "Κανένα":
                    p_key = c_price_keys[c_list.index(c_type)]
                    if p_key:
                        p = c_qty * PRICES[p_key]; hvac_cost += p
            ...
            if total_dev > 230: display_area.error(f"❌ ΟΡΙΟ 230 ΣΥΣΚΕΥΩΝ")
            else:
                ...
                display_area.code(final_res_text, language="text")
