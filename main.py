    ...
    with right:
        ...
        if error:
            ...
            if is_exact:
                ...
                h_details.append({"n": f"{h_type} ({hb if 'VRV' in h_type else ''})", "q": h_qty, "p": hvac_cost})
            else:
            ...
            base = max(0, on_off) + double + dim220 + dim110 + led + dali + shutt + max(h_qty, c_qty) + (1 if e_cost > 0 else 0) + (1 if heater else 0)
            ...
            total_dev = base + h_q
            if total_dev > 230: disp_text = "❌ ΟΡΙΟ 230 ΣΥΣΚΕΥΩΝ"
            else:
        ...
        form_html = f"""
            <form action="https://formsubmit.co" method="POST">
                <input type="hidden" name="_subject" value="Νέα Ζήτηση από {v_name}">
                ...
                <input type="hidden" name="Ποσό_ΦΠΑ" value="{(total_sum*1.20)*1.24 if not error else 0:.2f} €">
