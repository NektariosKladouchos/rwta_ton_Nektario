...
with tab_home:
    st.markdown("<h2 style='text-align: center;'>📸 Digital Showroom</h2>", unsafe_allow_html=True)
    c1, c2 = st.columns(2)
...
with tab_calc:
    ...
    with right:
        ...
        # ΑΡΧΙΚΗ ΕΜΦΑΝΙΣΗ ΜΕ ΜΗΔΕΝΙΚΑ
        if not v_name or not v_job or not v_addr:
            res = f"{'='*65}\n GEYER SMART HOME - ΠΡΟΣΦΟΡΑ\n{'='*65}\n"
            res += "⚠️ ΠΑΡΑΚΑΛΩ ΣΥΜΠΛΗΡΩΣΤΕ ΣΤΟΙΧΕΙΑ ΠΕΛΑΤΗ\n"
            res += f"{'-'*65}\nΣΥΝΟΛΟ ΣΥΣΚΕΥΩΝ: 0\nΚΑΘΑΡΗ ΑΞΙΑ: 0.00€\nΜΕ ΦΠΑ 24%: 0.00€\n{'='*65}"
            ...
            if h_type == "Ενδοδαπέδια" and c_type == "Ενδοδαπέδια Δροσισμός":
                ...
                if h_qty > 0:
                    ...
                    hvac_cost += p
                    name = f"VRV/VRF ({h_brand})" if h_type == "VRV/VRF" else h_type
                    hvac_details.append({"n": name, "q": h_qty, "p": p})
                if c_qty > 0:
                    c_keys = ["", "heat_thermostat", "fancoil_ctrl", "fancoil_ctrl", "vrv_interface", "split_ac"]
                    p = c_qty * PRICES[c_keys[c_list.index(c_type)]]
                    hvac_cost += p
                    name = f"VRV/VRF ({c_brand})" if c_type == "VRV/VRF" else c_type
                    hvac_details.append({"n": name, "q": c_qty, "p": p})
            ...
            if dim220 > 0: res += f"{'Dimming 220V':<30} | {dim220:<5} | {dim220*63.92:9.2f}€\n"
            if dim110 > 0: res += f"{'Dimming 1-10V':<30} | {dim110:<5} | {dim110*52.00:9.2f}€\n"
...
with tab_docs: st.subheader("📂 Τεχνική Βιβλιοθήκη")
with tab_contact: st.subheader("📨 Ρωτήστε τον Νεκτάριο")
