...
with right:
    st.markdown("""<div class='info-text'><h4 style='margin-top:0;'>🏠 Υπολογισμός Smart Home GEYER</h4>Φτιάξτε τα υλικά της ζήτησης και στείλτε μου email.</div>""", unsafe_allow_html=True)
    
    with st.expander("🏆 20 ΛΟΓΟΙ ΓΙΑ ΝΑ ΕΠΙΛΕΞΕΤΕ ΤΟ ΣΥΣΤΗΜΑ ΜΑΣ"):
        st.markdown("1. **Λειτουργία χωρίς Internet** | 2. **DALI, 1-10V, RGB** | 3. **Retrofit** | 4. **Z-Wave** | 5. **Alexa/Google** | 6. **Home Assistant** | 7. **Εύκολα Σενάρια** | 8. **Lua Scripting** | 9. **Local Backup** | 10. **Mesh** | 11. **Μεγάλη Γκάμα** | 12. **Installer App** | 13. **Energy Monitoring** | 14. **Ζώνες Θέρμανσης** | 15. **Πότισμα** | 16. **Notifications** | 17. **30% Εξοικονόμηση** | 18. **Ελληνική Υποστήριξη** | 19. **Design** | 20. **Αξία Ακινήτου**")
    ...
    if error:
        ...
        # ΔΙΚΛΙΔΑ ΑΠΟΦΥΓΗΣ ΔΙΠΛΗΣ ΧΡΕΩΣΗΣ HVAC
        h_c_hvac = 0; h_det = []
        if is_exact:
        ...
        # ΔΙΚΛΙΔΑ ΕΠΙΛΟΓΗΣ HUB ΒΑΣΕΙ ΣΥΣΚΕΥΩΝ
        ...
        if total_dev > 230:
            disp_text = f"{'='*70}\n❌ ΣΦΑΛΜΑ: ΥΠΕΡΒΑΣΗ ΟΡΙΟΥ 230 ΣΥΣΚΕΩΝ\n{'='*70}"
        else:
            ...
            if base_c <= 37: res += f"{'Κεντρική μονάδα (40)':<40} | 1    | {PRICES['hub_small']:10.2f}€\n"
            elif base_c <= 97: res += f"{'Κεντρική μονάδα (100)':<40} | 1    | {PRICES['hub_large']:10.2f}€\n"
