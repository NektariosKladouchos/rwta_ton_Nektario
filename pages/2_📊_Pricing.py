...
# ΣΗΜΕΙΩΣΗ: Δεν βάζουμε st.set_page_config εδώ, υπάρχει ήδη στο main.py
...
st.markdown("""
    <style>
    .stNumberInput, .stSelectbox, .stTextInput, .stRadio { margin-bottom: -15px !important; }
    .stMarkdown h3 { font-size: 16px !important; color: #1E3A8A; margin-top: 10px; }
    .display-box {
        background-color: #ffffff; padding: 15px; border: 2px solid #27ae60;
        border-radius: 8px; box-shadow: 0px 4px 15px rgba(0,0,0,0.1);
    }
    pre { font-family: 'Consolas', monospace !important; font-size: 12px !important; color: #000 !important; }
    </style>
...
with right:
    ...
    if not v_name or not v_job or not v_addr: 
        ...
        error = "❌ ΛΑΘΟΣ: Διαφορετικές μάρκες VRV σε Θέρμανση/Ψύξη."
    ...
    if not error and is_exact and h_qty != c_qty: 
        error = f"❌ ΛΑΘΟΣ: Η ποσότητα πρέπει να είναι ίδια σε {h_type}"
    ...
    if error:
        ...
        if is_exact:
            ...
            if h_qty > 0: 
                p = h_qty * PRICES[h_m.get(h_type, "heat_thermostat")]; h_c_hvac += p; h_det.append({"n": f"Θ: {h_type} {hb}", "q": h_qty, "p": p})
            if c_qty > 0: 
                p = c_qty * PRICES[h_m.get(c_type, "fancoil_ctrl")]; h_c_hvac += p; h_det.append({"n": f"Ψ: {c_type} {cb}", "q": c_qty, "p": p})
        ...
        if total_dev > 230:
            st.warning("⚠️ ΥΠΕΡΒΑΣΗ ΟΡΙΟΥ ΣΥΣΚΕΥΩΝ (Max: 230)")
        else:
            ...
            # Δημιουργία Κειμένου Προσφοράς
            res = f"{'='*60}\n GEYER SMART HOME - ΠΡΟΣΦΟΡΑ\n{'='*60}\n"
            res += f"ΠΕΛΑΤΗΣ: {v_name.upper()} | {v_job}\nΔΙΕΥΘΥΝΣΗ: {v_addr}\n{'-'*60}\n"
            res += f"{'ΠΕΡΙΓΡΑΦΗ':<30} | {'TEM':<4} | {'ΤΙΜΗ':>10}\n{'-'*60}\n"
            
            if base_c <= 37: res += f"{'Hub Small (<40)':<30} | 1    | {PRICES['hub_small']:10.2f}€\n"
            elif base_c <= 97: res += f"{'Hub Large (<100)':<30} | 1    | {PRICES['hub_large']:10.2f}€\n"
            elif base_c <= 130: res += f"{'Hub Large + Small':<30} | 1+1  | {PRICES['hub_large']+PRICES['hub_small']:10.2f}€\n"
            else: res += f"{'Hub Large x2':<30} | 2    | {PRICES['hub_large']*2:10.2f}€\n"
            
            if on_off > 0: res += f"{'On/Off Lighting':<30} | {on_off:<4} | {on_off*63.92:10.2f}€\n"
            if double > 0: res += f"{'Double Lighting':<30} | {double:<4} | {double*63.92:10.2f}€\n"
            if dim220 > 0: res += f"{'Dimming 220V':<30} | {dim220:<4} | {dim220*63.92:10.2f}€\n"
            if dim110 > 0: res += f"{'Dimming 1-10V':<30} | {dim110:<4} | {dim110*52.00:10.2f}€\n"
            if led > 0:    res += f"{'LED Strip Dimming':<30} | {led:<4} | {led*63.92:10.2f}€\n"
            if dali > 0:   res += f"{'DALI Lighting':<30} | {dali:<4} | {dali*160.00:10.2f}€\n"
            for d in h_det: res += f"{d['n'][:30]:<30} | {d['q']:<4} | {d['p']:10.2f}€\n"
            if shutt > 0:  res += f"{'Shutter Control':<30} | {shutt:<4} | {shutt*63.92:10.2f}€\n"
            if e_val > 0:  res += f"{f'Energy ({energy})':<30} | 1    | {e_val:10.2f}€\n"
            if heater:     res += f"{'Heater Control':<30} | 1    | {95.00:10.2f}€\n"
            res += f"{'-'*60}\n"
            res += f"{'ΣΥΝΟΛΟ ΥΛΙΚΩΝ:':<37} {total_mat:10.2f}€\n"
            res += f"{'ΦΠΑ 24%:':<37} {vat:10.2f}€\n"
            res += f"{'='*60}\n"
            res += f"{'ΓΕΝΙΚΟ ΣΥΝΟΛΟ:':<37} {gen_total:10.2f}€\n"
            res += f"{'='*60}\n"
            res += f"Προγραμματισμός: {prog_cost:.2f}€"
            disp_text = res
        ...
        notes = st.text_area("📝 Παρατηρήσεις:", placeholder="Στοιχεία επικοινωνίας...")
        ...
        if st.button("🚀 ΠΡΟΕΤΟΙΜΑΣΙΑ ΑΠΟΣΤΟΛΗΣ"):
            if not error:
                ...
                st.markdown(f'<a href="{mailto_link}" style="background-color: #27ae60; color: white; padding: 15px; text-decoration: none; border-radius: 5px; font-weight: bold; display: block; text-align: center;">📩 ΠΑΤΗΣΤΕ ΕΔΩ ΓΙΑ ΑΠΟΣΤΟΛΗ</a>', unsafe_allow_html=True)
