    ...
    with right:
        ...
        if error:
            ...
            if total_dev > 230:
                ...
                # --- PREMIUM ΠΙΝΑΚΑΣ ΠΡΟΣΦΟΡΑΣ ---
                res = ""
                res += "==============================================================\n"
                res += "                 GEYER SMART HOME - ΠΡΟΣΦΟΡΑ\n"
                res += "==============================================================\n"
                ...
                # --- ΜΕΤΡΗΤΗΣ ---
                if e_val > 0:
                    res += f"{f'Μετρητής Ενέργειας ({energy})':<40} | {1:<4} | {e_val:10.2f}\n"

                # --- ΘΕΡΜΟΣΙΦΩΝΑΣ ---
                if heater:
                    res += f"{'Έλεγχος Θερμοσίφωνα':<40} | {1:<4} | {95.00:10.2f}\n"

                # --- ΣΥΝΟΛΑ ---
                res += "--------------------------------------------------------------\n"
                res += f"{'ΣΥΝΟΛΟ ΣΥΣΚΕΥΩΝ:':<40} | {total_dev:<4} |\n"
                res += f"{'ΚΑΘΑΡΗ ΑΞΙΑ ΥΛΙΚΩΝ:':<45} {total_mat:10.2f}€\n"
                res += f"{'ΦΠΑ 24%:':<45} {vat:10.2f}€\n"
                res += "==============================================================\n"
                res += f"{'ΓΕΝΙΚΟ ΣΥΝΟΛΟ:':<45} {gen_total:10.2f}€\n"
                res += "==============================================================\n"
                res += f"{'ΚΟΣΤΟΣ ΠΡΟΓΡΑΜΜΑΤΙΣΜΟΥ (χωρίς ΦΠΑ):':<45} {prog_cost:10.2f}€\n"
